...
| ![聊天](docs/screenshots/06-chat.png) | ![扫码](docs/screenshots/05-qr.png) |
| **聊天 · Chat**<br>像 QQ 一样：左边会话列表，收发的文件就长在气泡里<br>*Conversation list on the left; files live inside the bubbles* | **手机连接 · Phone**<br>扫码即用，手机不装任何 App<br>*Scan and go — nothing to install on the phone* |
| ![设置](docs/screenshots/04-settings.png) | ![接收](docs/screenshots/03-receive.png) |
| **设置 · Settings**<br>主题（默认深色）、并行连接数、PIN、自动接收、加密状态<br>*Theme (dark by default), streams, PIN, auto-accept, encryption* | **接收 · Receive**<br>收到的文件、发送方与大小一目了然<br>*What arrived, from whom, and how big* |

> 截图摄于本机实跑，默认 1080×760 窗口。
> *Captured from a real run at the default 1080×760 window.*

界面用的是 **harness Web 客户端那套设计语言**：近黑画布（`#151517`）、发丝级描边、
近白正文，强调色是**单色**而不是另一个色相的蓝。三个客户端（电脑端 / 网页版 / 安卓 App）
共用同一组 token，**深色是默认主题**：网页版和 App 都没有浅色以外的选择，
电脑端可以在「设置 → 主题」里切浅色，或用 `EVERSEND_THEME=light` 启动。

*The three clients share one set of tokens taken from the harness web client — near-black
canvas, hairline borders, monochrome accent. **Dark is the default**; the desktop app can be
switched to light in Settings (or with `EVERSEND_THEME=light`).*
